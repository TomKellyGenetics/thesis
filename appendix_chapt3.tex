...
SLIPT expression analysis (described in Section~\ref{methods:SLIPT}) on TCGA breast cancer data ($n = 969$) found the following genes and pathways, described in sections~\ref{chapt3:exprSL_genes} and~\ref{chapt3:exprSL_pathways}.
...
\begin{tablenotes}
\raggedright \small
Strongest candidate SL partners for \textit{CDH1} by mtSLIPT with observed and expected numbers of \textit{CDH1} mutant \gls{TCGA} breast tumours with low expression of partner genes.
\end{tablenotes}
\end{threeparttable}
}
\end{table*}
...
\clearpage
\section{Compare SLIPT genes} \label{appendix:compare_mtSL}

The mutation synthetic lethal partners with \textit{CDH1} were also compared to siRNA primary screen data \citep{Telford2015}, by correlation and siRNA viability as described in sections~\ref{chapt3:compare_correlation} and~\ref{chapt3:compare_viability}.
...
\begin{tablenotes}
\raggedright \small
Strongest candidate SL partners for \textit{CDH1} by mtSLIPT with observed and expected numbers of mutant \textit{CDH1} \gls{TCGA} breast cancer tumours with low expression of partner metagenes.
\end{tablenotes}
\end{threeparttable}
}
\end{table*}
...
%
    ...
    \end{center}
    \caption[Somatic mutation against PIK3CA metagene]{\small \textbf{Somatic mutation against PIK3CA metagene.} Mutations in \textit{PIK3CA}, \textit{PIK3R1}, \textit{CDH1}, and \textit{TP53} were examined in TCGA breast cancer for their effect on the PIK3CA  \citep{Gatza2014} pathway metagene. The tumour suppressors \textit{CDH1} and \textit{TP53} showed an increase and decrease in the metagene respectively, whereas \textit{PIK3CA} and \textit{PIK3R1} mutations weaker evidence of decrease in metagene levels.
}
...
%
    ...
    \end{center}
    \caption[Somatic mutation against PI3K protein]{\small \textbf{Somatic mutation against PI3K protein.} Mutations in \textit{PIK3CA}, \textit{PIK3R1}, \textit{CDH1}, and \textit{TP53} were examined in TCGA breast cancer for their effect on the expression of the p110$\alpha$ protein (encoded by \textit{PIK3CA}). Protein levels were significantly elevated in samples with \textit{PIK3CA} or \textit{PIK3R1} mutations and lower in samples with \textit{TP53} mutations.
}
...
\begin{center}
  \resizebox{1 \textwidth}{!}{
   ...
   \end{center}
   \caption[Pathway metagene expression profiles]{\small \textbf{Pathway metagene expression profiles.} Expression profiles for metagene signatures from \citet{Gatza2014} in TCGA breast data, annotated for clinical factors and cancer gene mutations.
}
...
\begin{center}
  \resizebox{1 \textwidth}{!}{
   ...
   \end{center}
   \caption[Expression profiles for BRCA related genes]{\small \textbf{Expression profiles for BRCA related genes.} Expression profiles the genes contained in the gene signature related to \textit{BRCA1} and \textit{BRCA2} functions from \citet{Gatza2014} in TCGA breast data, annotated for clinical factors and cancer gene mutations. Samples are separated by \textit{CDH1} expression status and sorted by the metagene. In both cases, the majority of genes were consistent with the direction of the metagene, with few very exceptions. \textit{BRCA1} and \textit{BRCA2} mutant samples had higher metagene expression than most samples for the ductal subtype, although this was not the case (for the lobular samples for which the metagene was lower). However, the metagene was higher for basal subtype and estrogen receptor negative samples.
}
...
The intrinsic subtypes for \gls{TCGA} breast cancer samples provided by UCSC \citep{TCGA2012} that were derived from microarray analysis have been compared to the PAM50 results for performing subtyping from \gls{RNA-Seq} data \citep{Parker2009}. As shown in Table~\ref{tab:intrinsic_subtypes}, these subtypes were highly concordant for samples which had both procedures performed upon them ($\chi^2 = 1305.9$, $p = 2.73 \times 10^{-268}$). The main exception were the luminal A samples some of which were reclassified as luminal B or ``normal-like''.
...
\begin{tabular}{llllll}
 \cellcolor{white} & \multicolumn{5}{c}{\bfseries UCSC Subtype}  \\  
\cline{2-6}
...
\\
%\begin{tabular}{llllll}
 \cellcolor{white} & \multicolumn{5}{c}{\bfseries PAM50 Subtype}  \\
\cline{2-6}
 \cellcolor{white} & Basal-like &  HER2-enriched  &  Luminal A  &  Luminal B  &  Normal-like  \\
\cline{2-6}
\rowcolor{black!10} \cellcolor{white}
...
%\begin{tabular}{lllllll}
 \cellcolor{white} & \multicolumn{5}{c}{\bfseries UCSC Subtype}  \\
   \cline{2-6}
 \textbf{PAM50 Subtype} & Basal-like &  HER2-enriched  &  Luminal A  &  Luminal B  &  Normal-like  \\
   \hline
...
\begin{tablenotes}
\raggedright \small
The intrinsic subtypes of TCGA breast samples were compared between those provided by UCSC \citep{TCGA2012} from microarray expression to those derived from RNA-Seq data \citep{Parker2009}. Comparisons between these were limited to samples for which both data types were available.
\end{tablenotes}
\end{threeparttable}
}
}
...
The PAM50 subtypes are potentially more accurate given similarity of these subtypes and that the remainder of the subtypes were accurately recapitulated with \gls{RNA-seq} data. Furthermore, UCSC subtypes correctly identified $\sfrac{22}{22}$ normal samples as ``normal-like'' and PAM50 subtyping in \gls{RNA-Seq} data had a success rate of $\sfrac{112}{113}$ (including all of those identified from microarrays). Therefore the PAM50 subtypes (performed on a larger cohort of samples) are appropriate to use for further interpretation, superceeding the UCSC subtypes available for a limited set of samples.
...
The following results are a replication of the TCGA results (in Chapter~\ref{chap:SLIPT}) with stomach cancer data, using synthetic lethality (SLIPT) against \textit{CDH1} mutation.
...
\section{Synthetic Lethal Expression Profiles} \label{appendix:stad_exprSL_clusters}

Similar to the analysis of synthetic lethal partners in~\ref{chapt3:stad_exprSL_genes} breast cancer, the partners detected for low \textit{CDH1} expression in stomach cancer were also examined for their expression profiles and the pathway composition of gene clusters. Hierachical clustering was performed on SLIPT partners for \textit{CDH1} as showing in Figure~\ref{fig:slipt_expr_stad_mtSL}. Over-representation for Reactome pathways for each of the gene clusters identified is given in Table~\ref{tab:pathway_clusters_stad_mtSL}.
...
\rowstyle{\bfseries}
  ...
  \rowcolor{Cluster_Red!20}
  Viral mRNA Translation &  77 &  45 & $1.2 \times 10^{-89}$ \\ 
  \rowcolor{Cluster_Red!15}
...
\rowstyle{\bfseries}
  ...
  \rowcolor{Cluster_Red!20} 
  \textbf{Viral mRNA Translation} & $3.3 \times 10^{-98}$ & $>1.031 \times 10^{-5}$ \\ 
  \rowcolor{Cluster_Red!15} 
...
The following results are a replication of the TCGA results (in Appendix~\ref{appendix:mtSL}) with stomach cancer data, using synthetic lethality (mtSLIPT) against \textit{CDH1} mutation.

\section{Synthetic Lethal Genes and Pathways} \label{appendix:stad_mtSL_genes}

\begin{table*}[!ht]
\caption{Synthetic lethal gene partners of \textit{CDH1} from mtSLIPT in stomach cancer}
\label{tab:gene_stad_mtSL}
\centering
\resizebox{0.8 \textwidth}{!}{
\begin{threeparttable}
...
%\begin{mdframed}
  ...
  \resizebox{0.66 \columnwidth}{!}{
    ...
    \caption[Comparison of mtSLIPT in stomach to siRNA]{\small \textbf{Comparison of mtSLIPT in stomach to siRNA.} Testing the overlap of gene candidates for E-cadherin synthetic lethal partners between computational (mtSLIPT) and experimental screening (siRNA) approaches. The $\chi^2$ test suggests that the overlap is no more than would be expected by chance ($p = 0.872$). %A Venn diagram of all 16298 genes tested by both approaches.
}
\label{fig:Venn_allgenes_stad_mtSL}
%\end{mdframed}
\end{figure}
...
\rowstyle{\bfseries}
  ...
  Influenza Life Cycle & 1 & $>1.2349 \times 10^{-5}$ \\ 
   \rowcolor{Cluster_Red!15!Cluster_Blue!15}  
  Influenza Viral RNA Transcription and Replication & 1 & $>1.2349 \times 10^{-5}$ \\ 
   \rowcolor{Cluster_Red!20!Cluster_Blue!20}  
...
\begin{tabular}{sl^l^c^c^c^c^c}
\rowstyle{\bfseries}
 \cellcolor{white} Pathway & ID & Observed & Expected & $\chi^2$value & p-value & p-value (FDR) \\
  \hline
  \rowcolor{black!10}
  ...
  \rowcolor{black!10}
  \begin{tabular}[c]{@{}l@{}}Regulation of cytoskeletal remodeling and cell spreading by IPP \\ complex components \end{tabular} & 446388 & 8 & 10.1 & 19 & $7.46 \times 10^{-5}$ & $0.00651$ \\ 
  \rowcolor{black!5}
...
\begin{tablenotes}
\raggedright \small
Strongest candidate SL partners for \textit{CDH1} by mtSLIPT with observed and expected numbers of \textit{CDH1} mutant \gls{TCGA} stomach cancer samples  with low expression of partner metagenes.
\end{tablenotes}
\end{threeparttable}
}
\end{table*}
...
\begin{tablenotes}
\raggedright \small
Genes with the most candidate SL partners SLIPT in TCGA stomach expression data with the number of partner genes predicted by direction criteria and $\chi^2$ testing separately and combined as a SLIPT analysis. Where specified, the p-values for the $\chi^2$ test were adjusted for multiple tests (FDR).
\end{tablenotes}
\end{threeparttable}
}
\end{table*}
...
\begin{tablenotes}
\raggedright \small
Strongest candidate SL partners for \textit{CDH1} by SLIPT with observed and expected numbers of \gls{CCLE} stomach samples with low expression of both genes.
\end{tablenotes}
\end{threeparttable}
}
\end{table*}
