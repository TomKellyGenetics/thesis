...
Understanding synthetic lethality is important in cancers and has shown an impact clinical practice and patient outcomes for certain genes already. Thus this thesis aims to identify synthetic lethal gene pairs using public gene expression data. Accordingly, chapter~\ref{chap:methods_dev} aims to develop the methods to do so including a synthetic lethal detection methodology (\gls{SLIPT}) which has been released as an R software package. This chapter also documents the simulation and network analysis procedures developed to support the use of \gls{SLIPT} and perform analyses in throughout this thesis. 

This thesis also aimed to demonstrate \gls{SLIPT} methodology for analysis of \gls{RNA-Seq} gene expression data. Chapter~\ref{chap:SLIPT} aimed to do so by performing an analysis to identify candidate synthetic lethal gene partners of \textit{CDH1} in public breast and stomach cancer data \citep{TCGA2012, TCGA2014GC}. Chapter~\ref{chap:SLIPT} also aimed to demonstrate the biological relevance of these candidate synthetic lethal partners by identifying synthetic lethal pathways and comparing them with the results of an experimental \gls{siRNA} screen \citep{Telford2015}.
