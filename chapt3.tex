...
\begin{table*}[!ht]
\caption{Candidate synthetic lethal gene partners of \textit{CDH1} from \gls{SLIPT}}
\label{tab:gene_SL}
\centering
...
\begin{table*}[!ht]
\caption{Pathways for \textit{CDH1} partners from \gls{SLIPT}}
...
\begin{table*}[!hp]
\caption{Pathway composition for clusters of \textit{CDH1} partners from \gls{SLIPT}}
...
\begin{mdframed}
  ...
  \resizebox{0.66 \columnwidth}{!}{
    ...
    \caption[Comparison of \gls{SLIPT} to \gls{siRNA}]{\small \textbf{Comparison of \gls{SLIPT} to \gls{siRNA}.} Testing the overlap of gene candidates for E-cadherin synthetic lethal partners between computational (\gls{SLIPT}) and experimental screening (\gls{siRNA}) approaches. The $\chi^2$ test suggests that the overlap is no more than would be expected by chance ($p = 0.281$). %A Venn diagram of all 16298 genes tested by both approaches.
}
...
\begin{table*}[!hp]
\caption{Pathway composition for \textit{CDH1} partners from \gls{SLIPT} and siRNA screening}
...
\begin{mdframed}
  ...
  \resizebox{0.5 \columnwidth}{!}{
   ...
   \caption[Resampled intersection of \gls{SLIPT} and siRNA candidates]{\small \textbf{Resampled intersection of \gls{SLIPT} and siRNA candidates.} Resampling analysis of intersect size from genes detected by \gls{SLIPT} and siRNA screening approaches over 1 million replicates. The proportion of expected intersection sizes for random samples below or above the observed intersection size respectively, lacking significant over-represent\-ation or depletion of siRNA screen candidates within the \gls{SLIPT} predictions for \textit{CDH1}.
...
\begin{table*}[!htp]
\caption{Pathways for \textit{CDH1} partners from \gls{SLIPT} and siRNA primary screen}
...
\begin{table*}[!ht]
\caption{Candidate synthetic lethal metagenes against \textit{CDH1} from \gls{SLIPT}}
...
The synthetic lethal analysis of genes and pathways (previously described for TCGA breast cancer data) was replicated in TCGA stomach cancer. The accompanying data for \gls{SLIPT} and mtSLIPT analyses against \textit{CDH1} expression and mutation are in Appendices~\ref{appendix:stad_exprSL} and~\ref{appendix:stad_mtSL} respectively.
...
\begin{table*}[!ht]
\caption{Pathways for \textit{CDH1} partners from \gls{SLIPT} in stomach cancer}
...
\begin{table*}[!ht]
\caption{Query synthetic lethal genes with the most \gls{SLIPT} partners}
...
\begin{table*}[!ht]
\caption{Pathways for genes with the most \gls{SLIPT} partners}
...
\begin{table*}[!b]
\caption{Pathways for \textit{CDH1} partners from \gls{SLIPT} in CCLE}
...
\begin{table*}[!tb]
\caption{Pathways for \textit{CDH1} partners from \gls{SLIPT} in breast CCLE}
