...
The SLIPT methodology is amenable for use on expression data including pathway metagenes (as generated in section \ref{methods:metagene}). The suitability of the SLIPT methodology to application on public gene expression data will further be supported by simulation results in section \ref{results:simulation_2015} and Chapter 6, including comparison to other statistical methods. SLIPT results for \textit{CDH1} will also compared experimental screen results in a breast cell line \citep{Telford2015}, primary screen results are discussed in section \ref{chapt3:compare_SL_genes} and secondary (validation) screen results are presented in Appendix \ref{appendix:secondary_screen}.
...
\section{Detecting Simulated Synthetic Lethal Partners} \label{results:simulation_2015}
