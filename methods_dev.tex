...
%  \resizebox{\textwidth}{!}{
         \begin{center}
%
        \subcaptionbox{\small Simulated \glslink{gene expression}{expression} matrix}{%
        ...
        \subcaptionbox{\small Corresponding gene function calls}{%
...
%  \resizebox{\textwidth}{!}{
         \begin{center}
%
	\subcaptionbox{\small Simulated gene function with SL genes}{%
        ...
        \subcaptionbox{\small Query gene added with SL condition}{%
...
%  \resizebox{\textwidth}{!}{
         \begin{center}
%
        \subcaptionbox{\small Initial \glslink{gene expression}{expression} matrix}{%
            \label{fig:simulate_SL:first}
            %\includegraphics[width=0.5\textwidth]{{"/home/tomkelly/Documents/PhD Otago Uni/SL_Model/graph_sim_method/expr_mat_inhibiting".png}}
            \includegraphics[width=0.45\textwidth]{{"/home/tomkelly/Documents/PhD Otago Uni/SL_Model/graph_sim_method/expr_mat".png}}
        }%
        \subcaptionbox{\small Simulated \gls{synthetic lethal} dataset}{%
...
%  \resizebox{\textwidth}{!}{
         \begin{center}
%
        \subcaptionbox{\small Statistical evaluation \label{fig:simulation_Apr15ROC:Perf}}{%
        ...
        \subcaptionbox{\small \gls{ROC} curves\label{fig:simulation_Apr15ROC:ROC}}{%
        ...
        \subcaptionbox{\small Statistical performance (\acrshort{AUROC}) \label{fig:simulation_Apr15ROC:AUC}}{%
...
The sensitivity of \gls{SLIPT} as a binary classifier of \glspl{synthetic lethal} (in Figure~\ref{fig:simulation_Apr15ROC:Perf}) declined with higher numbers of \gls{synthetic lethal} genes to detect, although this is somewhat mitigated by higher sample sizes. The minority of true \gls{synthetic lethal} partners are more difficult to distinguish when there are more of them (with a weaker \glslink{gene expression}{expression} signal from each). While a reduction of the false positive rate could be achieved for moderate numbers of underlying \gls{synthetic lethal} partners, the number of partners to be detected in analyses of \glslink{gene expression}{expression} data is unknown. However, this simulation procedure is amenable to assessing the performance of \gls{SLIPT} across simulation parameters, \glslink{graph}{graph} structures, and comparisons to other approaches (as presented in Chapter~\ref{chap:simulation}).
...
%  \resizebox{\textwidth}{!}{
         \begin{center}
%
        \subcaptionbox{\small Input $\Sigma$ matrix parameter  \label{fig:simulation_May4SL:first}}{%
            \includegraphics[width=0.35\textwidth]{{"SL_Model_May15mvnorm_heatmap_4SL_cor_comp_top(1)".pdf}}
        }%
        \subcaptionbox{\small Simulated correlation matrix \label{fig:simulation_May4SL:second}}{%
            \includegraphics[width=0.35\textwidth]{{"SL_Model_May15mvnorm_heatmap_4SL_cor_comp_top(2)".pdf}}
        }%
        
        \subcaptionbox{\small Simulated \gls{gene expression}  \label{fig:simulation_May4SL:third}}{%
           \includegraphics[width=0.35\textwidth]{{"SL_Model_May15mvnorm_heatmap_4SL_cor_comp_top(4)".pdf}}
        }%
	\subcaptionbox{\small Simulated gene function \label{fig:simulation_May4SL:fifth}}{%
...
%  \resizebox{\textwidth}{!}{
         \begin{center}
%
        \subcaptionbox{\small Input $\Sigma$ matrix parameter}{%
        ...
        \subcaptionbox{\small Simulated correlation matrix}{%
        ...
        \subcaptionbox{\small Simulated \gls{gene expression}}{%
	...
	\subcaptionbox{\small Simulated gene function}{%
...
%  \resizebox{\textwidth}{!}{
         \begin{raggedleft}
%
        \subcaptionbox{\small Gene category in simulations}{%
        ...
        \subcaptionbox{\small Corresponding $\chi^2$ values}{%
...
%  \resizebox{\textwidth}{!}{
         \begin{center}
%
        \subcaptionbox{\small Statistical evaluation \label{fig:simulation_May15ROC:Perf}}{%
...
%  \resizebox{\textwidth}{!}{
         \begin{center}
%
        \subcaptionbox{\small $\chi^2$ testing without direction}{%
        ...
        \subcaptionbox{\small \gls{SLIPT} with directional criteria}{%
...
These results show that the performance of \gls{SLIPT} is appropriate for the analysis of \glslink{gene expression}{expression} datasets, where positively correlated genes are commonly occur, with the directional condition robustly improving the performance of \gls{SLIPT} across simulation parameters (compared to the $\chi^2$ test). Without assuming the underlying number of \gls{synthetic lethal} genes, \gls{SLIPT} will performed than the $\chi^2$ test alone, irrespective of the significance threshold as shown by \gls{ROC} analysis (in Figure~\ref{fig:mvtnorm_dir_compare2}). The directional \gls{SLIPT} methodology outperformed the $\chi^2$ test at detecting \gls{synthetic lethal} partners with even up to 100 \gls{synthetic lethal} genes.
...
%  \resizebox{\textwidth}{!}{
         \begin{center}
%
        \subcaptionbox{\small Activating \glslink{graph}{pathway} structure }{%
            \fbox{
            \includegraphics[width=0.45\textwidth]{{"/home/tomkelly/Documents/PhD Otago Uni/SL_Model/graph_sim_method/simple_graph".png}}
            }
        }%
        \subcaptionbox{\small Pathway structure with inhibitions \label{fig:simple_graph:second}}{%
            \fbox{
...
%  \resizebox{\textwidth}{!}{
         \begin{center}
%
        \subcaptionbox{\small Inhibiting pathway structure\label{fig:simulation_inhibiting:first}}{%
        ...
        \subcaptionbox{\small Distance matrix \label{fig:simulation_inhibiting:second}}{%
            \includegraphics[width=0.35\textwidth]{{"/home/tomkelly/Documents/PhD Otago Uni/SL_Model/graph_sim_method/dist_mat".png}}
        }%
        
        \subcaptionbox{\small $\Sigma$ (expected correlation) \label{fig:simulation_inhibiting:third}}{%
	...
	\subcaptionbox{\small Simulated correlation\label{fig:simulation_inhibiting:fifth}}{%
        ...
        \subcaptionbox{\small Simulated \glslink{gene expression}{expression} data \label{fig:simulation_inhibiting:fourth}}{%
        ...
        \subcaptionbox{\small Simulated gene function calls \label{fig:simulation_inhibiting:sixth}}{%
...
%  \resizebox{\textwidth}{!}{
         \begin{center}
%
        \subcaptionbox{\small Customised violin plot}{%
            \label{fig:vioplot:first}
            \includegraphics[width=0.45\textwidth]{{"vioplot1".png}}
        }%
        \subcaptionbox{\small Split violin plot}{%
